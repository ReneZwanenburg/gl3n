...
@safe pure nothrow vec3 rgbToHsv(vec3 inp)
{
...
{
    return vec4(rgbToHsv(vec3(inp.rgb)), inp.a);
}
...
unittest
{
    ...
    assert(rgbToHsv(vec3(0.0f, 0.0f, 1.0f)) == vec3(240.0f, 1.0f, 1.0f));
}
...
/// is also not pure.
@safe nothrow vec3 hsvToRgb(vec3 inp)
{
    if(inp.y == 0.0f)
	{ // s
...
{
    return vec4(hsvToRgb(vec3(inp.xyz)), inp.w);
}
...
unittest
{
    ...
    assert(hsvToRgb(vec3(300.0f, 1.0f, 1.0f)) == vec3(1.0f, 0.0f, 1.0f));
}